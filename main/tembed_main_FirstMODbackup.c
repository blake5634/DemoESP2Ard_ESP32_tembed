...
lv_obj_t *name_label;
char* names[] = {"blake", "cynthia", "alec", "sophia"};

static void knob_left_cb(void *arg, void *data)
{
    int xkn = iot_knob_get_count_value((knob_handle_t)arg);
    ESP_LOGI(TAG, "KNOB: KNOB_LEFT Count is %d", xkn);
    lv_label_set_text_fmt(count_label,"%d", xkn);
    int idx = (100+xkn)%4; // avoid neg values (hack)
...
{
    int xkn = iot_knob_get_count_value((knob_handle_t)arg);
    ESP_LOGI(TAG, "KNOB: KNOB_RIGHT Count is %d", xkn);
    lv_label_set_text_fmt(count_label,"%d", xkn);
    int idx = (100+xkn)%4; // avoid neg values (hack)
    char* namestr = names[idx];
    lv_label_set_text_fmt(name_label,namestr);
...
void lvgl_demo_ui(lv_disp_t *disp) {
    ...
    label = lv_label_create(cont_row);
    lv_label_set_text_static(label, "HH:MM AM");
    lv_obj_add_style(label, &l_style, LV_PART_MAIN);
 ...
 *     Time container row
 */
    /*Create a display container For a time:   HH:MM AM/PM  */
    lv_obj_t * cont_time = lv_obj_create(lv_disp_get_scr_act(disp));
    lv_obj_set_size(cont_time, 250, 50);
    lv_obj_align(cont_time, LV_ALIGN_BOTTOM_MID, 0, -25);
    lv_obj_set_flex_flow(cont_time, LV_FLEX_FLOW_ROW);
    lv_obj_set_style_bg_color(cont_time, lv_palette_main(LV_PALETTE_GREY), LV_PART_MAIN);

    /*Add items to the row*/
    label = lv_label_create(cont_time);
    lv_obj_set_size(label, 150, 20);
    lv_label_set_text_static(label, "Test Area");
    lv_obj_add_style(label, &l_style, LV_PART_MAIN);
    lv_obj_set_style_bg_color(label, lv_color_hex(0xff0000), LV_PART_MAIN);
    lv_obj_center(label);
/*
    ...
    label = lv_label_create(cont_time);
    lv_label_set_text_static(label, "MM");
    lv_obj_add_style(label, &l_style, LV_PART_MAIN);
    lv_obj_set_style_bg_color(label, lv_palette_main(LV_PALETTE_BLUE), LV_PART_MAIN);
    lv_obj_center(label);
    */
