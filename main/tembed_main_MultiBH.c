 ...
 *     click      | select focus   |  back to top lev.
 *                |                |
 *    ____________________________________________
 *     wheel      |   move         |  change
 *                |   highlight    |    value
...
enum level= {LEVEL_TOP, LEVEL_BUTTON}
lv_obj_t *count_label;
lv_obj_t *name_label;
char* names[] = {"blake", "cynthia", "alec", "sophia"};
lv_obj_t *focus_top_label;  // current focus at top level
...
static void knob_left_cb(void *arg, void *data)
{
  if (level == LEVEL_BUTTON){
        ...
        */
        ...
        ESP_LOGI(TAG, "KNOB: Change Button value: KNOB_RIGHT Count is %d", xkn);
        ...
        if foci_idx < 0;
            foci_idx = NFOCI-1;
        //  move highlight border(foci[foci_idx])
        currentFocus = foci[foci_idx];
...
static void knob_right_cb(void *arg, void *data)
{
    if (level == LEVEL_BUTTON){
        ...
        */
        int xkn = iot_knob_get_count_value((knob_handle_t)arg);
        ESP_LOGI(TAG, "KNOB: Change button: KNOB_RIGHT Count is %d", xkn);
        lv_label_set_text_fmt(count_label,"%d", xkn);
        int idx = (100+xkn)%4; // avoid neg values (hack)
        char* namestr = names[idx];
        lv_label_set_text_fmt(name_label,namestr);
...
static void button_press_down_cb(void *arg, void *data) {
    ...
    if (level == LEVEL_BUTTON){
        ...
        *    back to top level
        */
       ESP_LOGI(TAG, "Button Down! - exiting back to top level");
       ...
       level = LEVEL_TOP ;
         ...
         *  select focus
         */
        ESP_LOGI(TAG, "Button Down! - go into button level");
